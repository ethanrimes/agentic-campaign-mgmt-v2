// frontend/app/api/trend-seeds/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${SUPABASE_URL}/rest/v1/trend_seeds?${queryParams.toString()}`
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch trend seeds' },
    ...
    console.error('Error in trend-seeds API:', error)
    return NextResponse.json(
