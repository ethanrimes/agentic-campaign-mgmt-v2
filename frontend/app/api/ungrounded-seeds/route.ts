// frontend/app/api/ungrounded-seeds/route.ts
...
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${SUPABASE_URL}/rest/v1/ungrounded_seeds?${queryParams.toString()}`
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch ungrounded seeds' },
    ...
    console.error('Error in ungrounded-seeds API:', error)
    return NextResponse.json(
