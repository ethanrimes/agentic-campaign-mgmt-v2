...
class ContentCreationTaskRepository(BaseRepository[ContentCreationTask]):
    ...
    async def mark_in_progress(self, task_id: UUID) -> ContentCreationTask | None:
    ...
    async def mark_completed(self, task_id: UUID) -> ContentCreationTask | None:
    ...
    async def mark_failed(self, task_id: UUID, error_message: str) -> ContentCreationTask | None:
        """Mark a task as failed."""
        return await self.update(task_id, {"status": "failed", "error_message": error_message})
